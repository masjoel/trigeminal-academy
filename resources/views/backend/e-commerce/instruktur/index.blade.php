            ...
            <div class="section-body">
                ...
                <div class="row mt-4">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header header-elements">
                                @can('instruktur.create')
                                    <a href="{{ route('instruktur.create') }}" class="btn btn-primary"><i
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table-striped table">
                                        <tr>
                                            ...
                                            @can(['instruktur.edit', 'instruktur.delete'])
                                                <th class="text-center" width="120">Action</th>
                                            @endcan
                                        </tr>
                                        @foreach ($instruktur as $index => $item)
                                            <tr>
                                                <td width="50">{{ $index + $instruktur->firstItem() }}</td>
                                                <td width="50">
                                                    @if ($item->photo)
                                ...
                                <div class="mt-5">
                                    {{ $instruktur->withQueryString()->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        $(document).on("click", "a#delete-data", function(e) {
            e.preventDefault();
            let id = $(this).data('id');
            showDeletePopup(BASE_URL + '/instruktur/' + id, '{{ csrf_token() }}', '', '',
                BASE_URL + '/instruktur');
        });
    </script>
@endpush
