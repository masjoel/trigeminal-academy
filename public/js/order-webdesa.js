        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.data, function (index, item) {
                    ...
                    html += '<div class="d-flex" style="float:right">'
                    html += '<button class="mr-2 btn p-2 bg-warning" onclick="updateSubtotal(`' + JSON.stringify(item.id) + '`, `' + price + '`, `' + item.budget + '`, `minus`)" style="margin-right:10px"><i class="fas fa-minus"></i></button>'
                    html += '<button class="btn p-2 bg-default" onclick="updateSubtotal(`' + JSON.stringify(item.id) + '`, `' + price + '`, `' + item.budget + '`, `plus`)"><i class="fas fa-plus"></i></button>'
                    html += '</div>'
                    html += '</li>'
                });
